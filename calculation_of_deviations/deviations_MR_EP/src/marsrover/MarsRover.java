...
import marsrover.TestingSingleton;
...
public class MarsRover {
   ...
   if(TestingSingleton.getInstance().US[2])	{	
	...
	if(TestingSingleton.getInstance().US[7])	
	...
	public String executeCommand(String command){
		...
		//System.out.println( "Comando:" + command);
			for (int i=0;(i<command.length() & TestingSingleton.getInstance().US[5]);i++)
			{			
	...
	if(TestingSingleton.getInstance().US[6]) {		
				...
				auxObstaculo="";
				if ((encuentraObstaculo(x,y))==1 & TestingSingleton.getInstance().US[8]) //si encuentra obst�culo graba y vuelve a la �ltima posici�n
				{
					...
					if( ! contains( this.strObstaculo, auxObstaculo) ) {
						if(TestingSingleton.getInstance().US[9] & this.strObstaculo.length() > 0)
